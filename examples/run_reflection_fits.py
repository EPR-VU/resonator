"""Fit LinearReflectionFitter to S11 droplet CSV data and save results as SVG."""
...
# ── Configuration ─────────────────────────────────────────────────────────────
DATA_DIR = "/Users/b3-34/Projects/uni/bakalaurinis/matavimai/generated/environment"
OUT_DIR = "/Users/b3-34/Projects/resonator/resonator/examples/generated_fits/environment"

# Fractional half-width around the resonance dip used for windowing
FREQ_WINDOW = 0.001   # ±0.1 % of the dip frequency
...
for csv_path in csv_files:
    ...
    exp_Qi    = int(m.group(1))   if m else None
    exp_Qc    = int(m.group(2))   if m else None
    exp_phi   = float(m.group(3)) if m else None
    exp_delay = float(m.group(4)) if m else None
    exp_a     = float(m.group(5)) if m else None
    exp_alpha = float(m.group(6)) if m else None
